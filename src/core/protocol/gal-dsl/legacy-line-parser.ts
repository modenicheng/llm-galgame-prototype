/**
 * Gal DSL v1 line parser（LEGACY，冻结）— 解析一行完整、已 trim 的旧协议
 * 行（docs/llm-outputs-refactor.md §42）。
 *
 * C4 冻结边界：本模块只服务显式 legacy 请求/存档（protocolVersion 1），
 * 行为与 C4 之前逐字节一致；v2 请求永不进入本语法（不允许每行猜 v1/v2，
 * 也不向模型同时教两套）。新增语法一律写在 line-parser.ts 的 v2 解析器，
 * 不许回头改这里。唯一例外：两个被 v2 共享的**纯形状**助手
 * （matchEndSentinel / matchEndingEpilogueLine）只认行形状、不带版本语义，
 * 供 v2 复用以保证 @end/@ending 两版语义不漂移；本模块对它们的调用与
 * 原 内联逻辑 逐字节等价。
 *
...
// ---------------------------------------------------------------------------
// v1/v2 共享的纯形状助手（只认行形状，不带版本语义）
// ---------------------------------------------------------------------------

/**
 * `@end <nonce> <reason>` 哨兵形状（v1/v2 共用，冻结语义）。行不匹配该
 * 形状时返回 null（交回调用方继续分发）；形状匹配但 nonce/reason 非法时
 * 原样抛 SENTINEL_* 结构化错误。
 */
export function matchEndSentinel(line: string): { nonce: string; reason: SegmentEndReason } | null {
...
/**
 * `@ending <…>` 行形状（v1/v2 共用）：返回 @ending 之后的剩余原文
 * （token 解析见 interpretEndingEpilogue），非该形状返回 null。前视断言
 * 保证 `@ending_title`、`@endingHE` 之类粘连写法不误匹配——它们落进
 * UNKNOWN_COMMAND。
 */
export function matchEndingEpilogueLine(line: string): string | null {
  if (!/^@ending(?:\s|$)/.test(line)) return null;
 ...
 * the normalization entirely (conservative: no guessing).
 */
export function parseDslV1Line(rawLine: string, knownSpeakers?: ReadonlySet<string>): DslLine {
