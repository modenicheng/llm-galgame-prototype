...
describe("LocalQwen3TtsProvider", () => {
  ...
  it("dialect tts-server posts {text, voice} to /tts with header metadata", async () => {
    ...
    expect(seen[0]?.body).toEqual({ text: "好，我们从这里开始。", voice: "suyao" });
  ...
  it("shares the provider error class with the dashscope adapter (single type)", async () => {
    const err = new TtsProviderError("http_500", "boom");
    expect(err.code).toBe("http_500");
    expect(err).toBeInstanceOf(Error);
    // the class is the same object re-exported by both provider modules
    const dashscope = await import("./dashscope-cosyvoice-provider.js");
    expect(err).toBeInstanceOf(dashscope.TtsProviderError);
  });
});
