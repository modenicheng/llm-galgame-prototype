...
export class PlaybackBuffer {
   ...
   * removeLineIds). All concurrent waiters share ONE promise, and a waiter
   * that arrives after a mutation waits for the NEXT one — no accumulation
   * of stale race losers. Lets callers await buffer levels event-driven
   * instead of polling (e.g. the start-threshold gate).
   */
