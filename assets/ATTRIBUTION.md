...
- 以项目成员实拍的校园照片为底，经自配图像生成 API（gpt-image 系 `edit`，
  ...
  - `playground_*`：学校操场看台实拍（2026-09-18，竖直参照测得滚转
    约 0° 免旋转，直接裁 16:9；脚本 `output/campus_bgs2.mts`，源照片为
    黄昏照故基准时间=evening 再派生 day/night）；
  - `north_canteen_*`：北食堂（食堂②）门口街景实拍（2026-09-18，同上管线与脚本）；
