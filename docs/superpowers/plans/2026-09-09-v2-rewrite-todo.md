...
- [x] **M4.5 game.ts 拆分**（2707 → 1588 行，偏差记附录 B）
  落地（2026-09-17）：交互驱动簇 16 方法 ~900 行（handleChoice/handleInteractionInput/handleHybridInteraction/adoptSelectedBranch/createBranchManager*/bridge prefetch/recordPlayer*/stageResponseEvent/makePlayerDialogue/countBufferedDialogues/resolveInteraction）+ live 消费簇（consumePlayableEvents/consumeLiveSelection/consumeLiveInputResponse/consumeLiveStream/buildRuntimeInteraction）经 `InteractionHost` 接口移入 `src/runtime/interaction-driver.ts`；段类型（ActiveSegment/SegmentOutcome 等）迁至 `src/runtime/segment-types.ts`；Game 以 `interactionDriver` 委托，行为零变化（既有测试仅调用路径变化）。
  验收偏差（附录 B）：game.ts 1588 行达 ~1500 目标但仍在 900 源码阈值上（运行循环+段生命周期+恢复+图提交为卡片保留项），豁免保留并注明；interaction-driver.ts 1171 行待二分为 input/choice 两文件后移除豁免。
...
- [x] **M3.5 结局驱动收束 + event mode 删除**
  ...
  落地（2026-09-17）：① `DirectorService` 增可选 `outline: OutlineStorePort`，`refreshDirective` 的 `endingPressure` 取「模型判定 ∨ 大纲确定性信号」（信号 = 维护已 activate/realize 某 ending 候选，或前沿 act 全 realized 且尚有 ending 候选；大纲未加载/读取失败 → false 只告警不阻塞）；bootstrap 显式世界时把 `outline.store` 接入导演。收束指令进 prompt 既有链路：directive → `makeBriefing` → `buildActorBriefing`（「- 收束：剧情接近终章，向结局推进」行，actor-briefing.test 断言在册）→ 演员生成请求 `briefing` 字段。② forced ending 机构整体删除：game.ts 的 forceEnding/forcedEndingRetries/interactionCount/concludeWithRuntimeEnding/FORCED_ENDING_REPAIR_REASON、SegmentOutcome/ContinuationRequest 的 endingRequired、config `narrative.mode`+`narrative.event` 键、bootstrap mode 门控（导演无条件装配）、game-dsl 强制收束三测试与 bootstrap event-mode 测试。③ config 负面断言：yaml 含 `narrative.mode: event` 加载不报错且键被 zod strip（死键不生效）。验收三绿（1448 测试）。

- [x] **M3.6 canon 存储 + 晋升流程**
...
- [x] **M5.6 图维护（不可达内容 GC；无玩家删除入口）**
  ...
  落地（2026-09-17）：① `collectGarbage()`（coordinator 内部，restoreOrCreateRun 时机自动执行）：保护集 = 全部周目路径逆向行走（结局边按 run 全量枚举——同一结局 id 多周目重号场景必须逐边保护 / abandonedAt / 游标 / retrace 起点），不可达决策删除后再核对其牵连边，迭代至不动点（幂等）；payload 孤儿清理（discardOrphanPayloads）随边删除联动。③ 存储层：`parseJsonl` 支持 tombstone 行（`{id, deleted:true}`，latest-wins 折叠过滤；磁盘 append-only 不回改，§3 契约 schema 未动）；`GraphStorePort.removeDecision/removeEdge`（removeEdge 联动 deletePayload）。②④ 负面测试锁定 + 无玩家 UI（未新增任何删除入口）。
...
- [x] **GH-P3**：世界生成无确认闸门行为核对（POST /api/worlds 直通换绑，无确认步骤）；outline 冻结原则测试在位（OutlineStore 状态机/原子写/日志/损坏抛错 9 例）。（2026-09-17 过门：三绿 + 机械检查 + subagent 评审 18 条，P2 当场修复、P3 记附录 B）
...
- 2026-09-17（GH-P6 卫生门）：subagent 只读评审 25 条（P1×1、P2×7、P3×17，范围 44d3092..HEAD）。**P1 当场修复**：resumeRestoredInteraction 的 hybrid/input 分支透传 fastForward（此前仅 choice 分支转换——恢复表单命中快进后游标已前移而运行循环按空 preview 续跑，陈旧事件滞留 + seq 槽位复用）。**P2 当场修复**：① parseJsonl 折叠后 listRuns 的「最近写入序」契约修复（新增 parseJsonlWriteOrder 按各 id 最后写入位排序，兼修 idOf 死映射 P3）；② 评价喂回生产接线——buildGraphCoordinator 补传 reviewStore（此前 reviews 读取与 adapter 渲染为死代码）；③ graph.build 补传 stats（此前 outlineReview 生产恒不返回）；④ buildGraphView 拆出 actWhitelistByScene/sceneViewsOf/outlineReviewOf/endingLabel。**长度类 P2 记档（GH-P4 已挂账延续）**：consumeActiveSegment ~150 行、handleInteractionInput ~185 行、handleHybridInteraction ~125 行（含 syntheticChoice ×2）——interaction-driver 二分清偿时一并处理。**P3 记档**：listScenes/getRun 等六处 latestById 重复派生、逆向行走循环 ×4 抽 walkBackEdges、endingLabel 旧调用点收敛、web.ts reviews.submit 与 buildSettlementView 三段派生抽 locateLatestEnding、atomicWriteJson 仓内第 8/9 份、host 三 GET 路由抽 serveJsonRoute、retrace/gc 测试助手 ×5 移 test-helpers、host 测试 describe 嵌套与缩进、game.ts record 返回值双通道死支、settlement 闭包捕获旧 app 的 sessionId（世界换绑后 payoffRate 静默缺失——由宿主传参已修，记录于评审）、segment-types ChoiceSelection 重抄字段、line-parser 冗余三元、fast_forward 字面量 ×4、run()/boot()/startActiveSegment 超长（既有挂账）。
- 2026-09-17（GH-P5 卫生门）：subagent 只读评审 23 条（P1×0、P2×2、P3×21，范围 8aa0cbf..94615e0）。**P2 当场修复**：① M2.4 的 D8 outlineLocation 预筛键接通——migrateOutlineForScene 确定前沿 ref 后 putScene 回写场景节点 outlineRef（此前恒指 ol_seed 且 ol_seed 不在 outlineNodes，键永不生效），indexDecision 移到 migrate 之后并经内存映射（sceneOutlineRefs + outlineNodes）解析，与 hydrate 同源；② createRuntimeApplication 拆出 `buildDirectorService`/`buildCanonPromoter` 具名装配函数。**随手清（本阶段自留 debris 6 条）**：game.ts/story-generator-port/openai-compatible-generator 三处 Event mode 孤儿注释、coordinator 重复 EndStateKey 文档块、director-service narrowFormModes 格式破坏与死局部计算。**P3 记档（待后续卡清偿）**：coordinator sceneOutlineRefs Map 值未读（可 Set）、hydrate 双重 get、applyConfluenceMatch endState no-op 重申、loadCanonQuietly 死防御 catch、director-service 空指令字面量 ×3、readSceneHistory 静默 continue 无 warn、canon-store saveScaffold catch-all 应 ENOENT-only、canon-store-port getCanon throw 契约迫使调用方吞错（宜 `| undefined`）、canon prompt 渲染三处近似重复、OpenAI client 构造样板 ×5、outline-writer-adapter maintainOutline 解析失败与 schema 失败混报无告警、bootstrap narrativeDirector let+条件 spread 死分支、GameGraphStore 同 game 三处构造、world-generator seedStoryLine 链无消费方、isEnoent 惯用法仓内 ≥5 处。**豁免**：run-graph-coordinator.ts 958 行（M2.4 汇流子系统并入所致；拆出 confluence-checker 后移除，清偿：下一迭代）。
- 2026-09-17（M5.6 落地）：存储层 tombstone 行 `{id, deleted:true}` 为 JSONL 适配层扩展（`parseJsonl` 折叠语义：同 id 取最后出现，最终态 deleted → 过滤；磁盘 append-only 不回改），§3 契约 schema（core/graph/types.ts）未动——按卡面「记附录 B」要求在此登记。GC 保护行走发现并修复一处在先缺陷隐患：同一结局 id 被多个周目到达时保护集必须逐结局边枚举（首个匹配会导致最新周目路径失去保护）。
...
- 2026-09-17（GH-P4 卫生门）：subagent 评审 25 条。**当场修复**：P1——DirectorService 的场景指令生产循环接线至 Game reconcile 场景边界（triggerDirective fire-and-forget，相位门/防守节拍/场景目标生产可达）；P2——narrowFormModes 改闭包绑定 sceneId（删除共享可变字段 currentDirectiveSceneId）；actor-briefing 删除与 renderMemoryProjection 重复的 relatedFacts/characterBeliefs/avoidanceLessons 三字段（契约回归）。**P2 长度类与 P3 记档（下一迭代优先清偿）**：game.ts run()/consumeActiveSegment()/startActiveSegment() 超长与 run() buffer 分支 5 层嵌套；interaction-driver handleInteractionInput/handleHybridInteraction 拆分；web boot() 198 行；briefing 条件展开 ×7、syntheticChoice 构造 ×5、generateBranchPrefetch 组装 ×3、setResponseJob ×5 同构；未使用导入（InputResponseSession/InputCommitOutcome/ChoiceSelection/LiveStreamLike）、孤儿文档注释、EndOutcome/BufferOutcome 死导出、RuntimeBufferEvent 内联 import 类型、cli printMetrics 死参数等。
- 2026-09-17（GH-P3 卫生门）：subagent 只读评审 18 条（P2×2、P3×14、竞态/边界观察各 1，无 P1）。**P2 当场修复**：① web 首屏创建表单从 boot()（245 行）抽出 `mountWorldCreateForm`/`submitWorldCreation`（5 层嵌套→早返回压平）；② 大纲后台维护落盘入互斥链（applyOutlineQuietlyInner viaChain）——修复维护与确定性迁移交错重算 revision 的竞态。**顺手 P3**：json 记忆存储/世界生成 tmp+rename 复用统一；hasWorld 改用 GAME_STORAGE_LAYOUT 常量并把旧世界（无大纲）计入；cli/prompts/world-generator 的小型冗余清理；测试 rmSync 路径修正。**P3 记档（待后续卡清偿）**：跨 adapter 抽共享 `callJsonModel` 基础设施（5 处同构调用）；全仓 `isEnoent`/`writeFileAtomic` 公共谓词与工具抽取；outline-store applyRevision 三段迁移字面量合并；create-runtime-application 函数体仍 ~165 行（继续下沉 buildGameFor）；cli printMetrics 死参数。
...
- 2026-09-18（campus 线移植批次 feat/port-dsl-repair，依据协作者 handoff-2026-09-17 §2）：三组 P0 落地——P0-C 舞台状态注入保真化 + REDUNDANT_STAGE_CUE（campus 3414a39）；P0-A @ 前缀指令语法 + closing-repair 三层修复 + DslProtocolError 结构化 detail + reconcile/summarize 幻影角色去污染（ee1e1ea+82f0ac6+e92836f parser 部分）；P0-B 生成器剔除续写（strip-and-continue：白名单坏行剔除 + assistant 前缀同 parser 续写 + S1/S2/N1 边界加固）+ game.ts fail-fast 修复启动（e92836f+97798db+6365683）。**移植裁定**：group-builder 取 campus 终版（ending_epilogue 分支与 S1/N1 不变式同文件交织，随 A 整体落地）；`@ending` DSL 能力保留 parser/validator 惰性支持但协议/DSL_COMMAND_LIST 不宣传（handoff 产品决策类，待 v2 拍板，M5.5 通关评分如需结局档位可顺带启用）；recap 接管 recent_summary 不移植（v2 无 recap 管线）；stream usage 折叠/monitor 上报裁剪。**卫生门（完整档）**：机械检查通过（openai-compatible-generator.ts 1170 行登记豁免，清偿=拆 dsl-stream-attempt 模块）；subagent 只读评审 P1×0、P2×7、P3×9——P2 全部当场修复（协议文案许晚晴→苏遥、DslSourceLocation/source 死类型删除、raspberry 修复文案与注释/fixture 名词中性化、docs §110 裸语法场景 @ 化、ENDING_EPILOGUE_ORPHAN 白名单注释对齐、尾行路径 swap 守卫补 @ 行豁免、DSL_COMMAND_LIST 去 @ending）。**P3 记档**：@ending 惰性机制 v2 无运行时消费者（启用时机随 M5.5 评分）、group-builder 头注释旧 `?`…`/?` 写法、closing-repair.ts 复制痕迹空格、测试 mock 的 state_patch 冗余键（main 既有 30 处同款）、maybeStartEarlyRepair 的 diagnostics.info 在 try 外浮动 promise、提前启动的修复段 restart/shutdown 时成孤儿生成任务（与 pendingRefillSegment 同款既有语义）、llm.test 三选一正则稍宽松、compiler REDUNDANT 预检每 cue 双调纯 reduce（成本可忽略）。

- 2026-09-18（本地 TTS 移植批次 feat/port-local-tts，campus 线 72aed79 起）：LocalQwen3TtsProvider 落地（双方言 openai/tts-server、首块超时、abort bytes-so-far）+ voices.ts local binding（注册表键 voice、归一化 dashscope 形状 instruction_mode=none、validateLocalModelConfig 拦 sample_rate≠24000）+ selectTtsProvider local 分支与 config/factory 枚举扩展 + voices.yaml suyao/linche local 块与指南「五、本地合成」章节。**移植裁定**：campus 的 dashscope qwen3-tts 双协议（tts-model-family、validateDashscopeModelConfig）不移植——纯 local 部署下启动校验强制全局 24000，无按族分采样率需求；TtsProviderError 抽 tts-provider-error.ts 共享（campus 两侧重复定义，消费方鸭子类型零风险）；deferred 助手随卫生门抽 deferred.ts（mock/dashscope/local 三处重复）。**卫生门（完整档）**：机械检查通过（标记 11<基线 19）；subagent 只读评审 P1×0、P2×1、P3×5 全部当场修复（start() 拆 buildDialectRequest/pumpSession、pumpSession 已中止兜底、validateLocalModelConfig 单条聚合报错、local env 覆盖启动校验（空 base 回退默认/非 http(s) 拒绝/未知 dialect 响亮报错）、openai 方言 sampleRate 回退分支真实测试）。基线 1600 测试 / 118 文件。registry 键 suyao/linche 对应的参考音需在部署机 tts-server 注册表构建（机器本地资产，404 为运行时 provider 错误——设计如此，非缺陷）。

- 2026-09-16（角色音频特征批次 feat/voice-design，spec：2026-09-16-character-voice-design）：V1 导演层（SceneDirective.voice + performance-compiler 合并链 direction > perf > baseline > identity + VoiceDirectionHub 桥按 game.currentSceneId 重绑 + 导演音频调色板段）与 V2 编剧层（DraftCharacter.voice → world/voice-design.json 世界创建期写一次 → mergeVoiceDesignViews 注入动态角色 + characters.txt 嗓音行）落地。**能力边界（声明设计）**：dashscope 动态角色走共享回退音色 `DASHSCOPE_VOICE_FALLBACK`（空 = 无声）；local 动态角色 V3 前不注入（注册表键身份缺口）；音频指导按最近场景近似（预取前瞻 1-4 行，MediaPlannerPort 不改合同）；characters.txt 嗓音行进 system prompt（演员可见画像，有意）。**卫生门（完整档）**：机械检查通过；subagent 只读评审 P1×1、P2×1、P3×7 全部当场修复——P1 voice-design-store 的 avoid/baseline 误必填（合法最小画像 load 必炸）→ CharacterVoiceDesignSchema 提为 outline-writer.ts 唯一真源两侧共用；P2 注入角色调色板重复并集 → Set 去重；P3 导演 prompt 补 pace/energy 词表、双 prompt 词表改常量拼接、pickEnum 收敛四处同构校验、编剧 delivery schema 约束到枚举（词表外语义标签会静默过滤该角色全部 delivery）、factory 查表键改 characters 命中键（byName 回退显示名失配）、buildVoiceViews/buildSpeakerPalette 抽具名装配函数、spec §4.2 措辞对齐（directive 缓存跨 restart 延续与 formModes 同语义）。**附带发现**：create-runtime-application.test 的 compiler 整模块 mock 缺新增常量导出致 14 例静默不收集（总数 1629→1615 才暴露）→ 改 importOriginal 部分桩。基线 1629 测试 / 121 文件。V3（身份合成：qwen3-tts-vd / local 参考音中继 + per-game voice-bindings）待实施，config 门控默认关。
