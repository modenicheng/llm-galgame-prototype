...
- **左侧最大块**：编剧 LLM。全部开场/续写/分支预取/输入回应/过场桥接和
  修复重试按时间合并为一份连续 DSL 文档，不再逐段切换。每次请求的边界
  显示状态、开始时间、总耗时、首字延迟、输入/输出/缓存 token、字符、行、
...
- **右下 tabs**：
  ...
  - **剧情状态**：StoryState 投影（场景/角色/开放线索/canon/[Recap]/玩家
    画像）。
  - **指标**：MetricsSnapshot 卡片（请求计数、token、延迟分位、预取命中、
    校验失败、玩家时序样本、资产诊断）。
