...
交互题干以 `[交互] …`（截断 ~80 字）进入历史：它是模型自己提出的问题，
滑窗滑过后若看不到，模型会忘记哪些话题已被提出/解决——这是"不知道该
收束"的直接成因之一。choice / end 仍是纯机器事件，保持跳过。
...
交互进度、任务指令、修复原因、收束指引）。**禁止把逐请求变化的内容插到
稳定段之前**，否则会整体打破 DeepSeek 等厂商的前缀缓存。收束指引
（L1 wrapup / L2 closing / L3 endingRequired）一律追加在末尾。
Game 侧历史窗口（`generationHistory`）≤ `history_events` 时全量传递、
超限后按 20 条对齐滑动，保证两次滑动之间前缀逐字节稳定；生成器只在
超过 4×cap 时兜底再切片。
