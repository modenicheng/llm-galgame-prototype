...
`beat`/`?`/`+`/`=`/`/?`）已废弃：解析器对它们响亮报 `RETIRED_ALIAS`
（2026-09-17 第二层硬化），提示词只教 `@` 形式。
以 `@` 开头但无法识别的行直接报 `UNKNOWN_COMMAND`，绝不静默降级为旁白或
台词（历史事故：`@¬end 4607 buffer` 被当旁白播出、`@ch raspberry: …`
造出幻影发言人并经 StoryState 回流进后续 prompt）。行首全角 `＠` 与全角
`？＋＝／` 归一化为半角后再走 `@` 语法；正文行以全角 `？` 开头仍是旁白，
不受影响。
...
**例外：确定性补哨兵（移植自 campus 线 feat/end-sentinel-low，2026-09-18）。**
同时满足以下条件时，不判截断，本地合成 `@end <nonce> <reason>` 补完：
...
（campus 28 局实测占全部写手失败的 ~90%+）；而 fail 路径的代价是桥接旁白
整段废弃、分支预取降级为选中时重造。补哨兵后该类任务的漏哨兵失败率
campus 实测降为 0。
...
**舞台状态序列化契约（2026-09-17 保真化，自 campus 线移植）**：`serializeVisualContext`
对所有维度**显式**陈述，包括"无"——背景未设置输出 `背景：无（尚未设置）`，
BGM 已停/未播输出 `BGM：无（当前没有音乐播放，不要再输出 @bgm stop）`，台上无人输出
`角色：台上无人`。早期版本对"无"维度整行省略，模型无法区分"已停止"与"从未播放"，
是续写段重复输出 `@bgm stop` 的直接根因之一。块首带一行"只输出发生变化的指令"的
现状说明；`roster`（注册角色表）追加 `不在场：…` 行，供登台/撤离参照；隐藏角色
标注 `需 @ch <id> show 恢复`。
...
**段落顺序（2026-09-16 起，决议 D9）**：user prompt 按「稳定 → 易变」排序，使
相邻请求共享尽可能长的前缀（provider 前缀缓存友好）：

```text
剧情历史（单调追加；回溯 = 恢复重放天然截尾）
可用素材（每世界静态）
当前故事状态 / 导演便签 / 当前舞台状态（每回合变化）
任务类型 / 当前回合 / 目标行数 / 生成段 nonce / 任务指令（每请求变化，置尾）
```

历史区不做事件数窗口截断（`game.history_events` 已删除）：截断或摘要会移动公共
前缀起点，反而破坏缓存局部性；单周目全量历史远小于现代模型上下文。极长流程的
逃生舱是**场景锚定的 compact**（对上一场景节点之前的内容做摘要替换，锚点粒度
= 场景边界），不是事件数滑窗。
