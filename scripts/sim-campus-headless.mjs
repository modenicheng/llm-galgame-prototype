...
async function runSession() {
  ...
  console.log("\n──────── 会话汇总 ────────");
  console.log(`persona=${report.persona} seed=${report.seedEnv ?? "(按session轮换)"} 耗时=${(report.elapsedMs / 1000).toFixed(0)}s`);
  ...
  console.log(`LLM 请求：${JSON.stringify(report.metrics.llm.requests)} token(in/out/cached)=${report.metrics.llm.tokens.input}/${report.metrics.llm.tokens.output}/${report.metrics.llm.tokens.cached_input} 命中率=${(report.metrics.llm.cache_hit_rate * 100).toFixed(1)}%`);
  console.log(`延迟 p50/p95/max=${report.metrics.llm.latency_ms.p50}/${report.metrics.llm.latency_ms.p95}/${report.metrics.llm.latency_ms.max}ms schema失败=${report.metrics.errors.schema_validation_failures}`);
